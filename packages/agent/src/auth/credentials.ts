 ...
 *
 * OpenAI Codex / ChatGPT subscription tokens are also CLI credentials. Codex
 * subprocesses authenticate through the Codex CLI account store, not through
 * `process.env.OPENAI_API_KEY`. A direct OpenAI API key should remain the only
 * path that enables normal @elizaos/plugin-openai runtime inference.
 */
export async function applySubscriptionCredentials(config?: {
  ...
  //
  // Codex subscriptions power spawned `codex` CLI subprocesses. Do not inject
  // their OAuth access tokens into OPENAI_API_KEY: the normal OpenAI API path
  // expects scoped API keys and will fail with permission errors for CLI
  // subscription tokens.
  ...
  // Auto-set model.primary only for subscription providers that can power the
  // normal runtime provider path. CLI-backed subscriptions should not point the
  // runtime at direct API-key plugins.

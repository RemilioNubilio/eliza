...
async function doEnsureStewardAgent(
  ...
  try {
    ...
    try {
      ...
      // 404 = agent not found, 401/403 = no valid credentials yet.
      // All are expected on first run before tenant/agent provisioning.
      const status =
        err instanceof StewardApiError ? err.status : undefined;
      const isAuthError =
        status === 401 ||
        status === 403 ||
        (err instanceof Error &&
          /api key|unauthorized|forbidden/i.test(err.message));
      if (status !== 404 && !isAuthError) {
        throw err;
      }
    }

    // Agent doesn't exist or auth failed — try to create tenant + agent
    console.info(`[steward] Agent "${agentId}" not found or auth failed, provisioning...`);

    let tenantId = normalizeEnvValue(env.STEWARD_TENANT_ID);
    let apiKey = normalizeEnvValue(env.STEWARD_API_KEY);
    const masterPassword = normalizeEnvValue(env.STEWARD_MASTER_PASSWORD);

    // Auto-generate tenant credentials if master password is available but
    // tenant credentials are not. This enables zero-config steward setup.
    if (!tenantId && masterPassword) {
      tenantId = `desktop-${crypto.randomUUID().slice(0, 8)}`;
      env.STEWARD_TENANT_ID = tenantId;
    }
    if (!apiKey && masterPassword) {
      apiKey = `stwd_${crypto.randomBytes(32).toString("hex")}`;
      env.STEWARD_API_KEY = apiKey;
    }

    // Try to create tenant first (may already exist, that's ok)
    if (tenantId && apiKey && masterPassword) {
      try {
        ...
        if (!tenantRes.ok) {
          ...
          console.info(`[steward] Tenant "${tenantId}" created`);
        }
      } catch (tenantErr) {
        console.warn(
    ...
    // Reset the singleton lock so the next request retries provisioning
...
function persistAgentCredentials(
  ...
  try {
    ...
    applyStewardWalletAddressesToRuntimeCache(
      result.walletAddresses.evm ?? null,
      result.walletAddresses.solana ?? null,
    );
  } catch (credErr) {
    console.warn(

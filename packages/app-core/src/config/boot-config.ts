 ...
 * module without loading the React runtime.
 */
...
export function setBootConfig(config: AppBootConfig): void {
  ...
  // Shallow merge so Electrobun `apiBaseUpdate` (preload) can set apiToken/apiBase
  // before React mounts without this call wiping injected values.
  const next: AppBootConfig = { ...store.current, ...config };
  store.current = next;
  const globalObject = (
    typeof window !== "undefined"
      ? (window as unknown as Record<PropertyKey, unknown>)
  ...
  globalObject[BOOT_CONFIG_WINDOW_KEY] = next;
}

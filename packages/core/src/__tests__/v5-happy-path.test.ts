...
describe("v5 happy path — message handler → planner → executor → evaluator", () => {
	...
	it("only exposes actions whose validate passes before planner tool selection", async () => {
		let spawnCalls = 0;
		const unavailableSearch = makeMockAction({
			name: "SEARCH",
			contexts: ["general"],
			validate: async () => false,
			handler: async () => {
				throw new Error("SEARCH should not execute");
			},
		});
		const spawnAgent = makeMockAction({
			...
			handler: async (_runtime, _message, _state, options) => {
				...
				expect(options.parameters).toMatchObject({
					task: "Look up the current BTC price and summarize it briefly.",
				});
				return {
					success: true,
					text: "",
		...
		const runtime = makeRuntime({
			actions: [unavailableSearch, spawnAgent],
			responses: [
				{
					expectModelType: ModelType.RESPONSE_HANDLER,
					body: JSON.stringify({
						...
						simple: false,
						contexts: ["general"],
						thought: "This needs a tool-capable agent.",
					}),
				},
				{
					expectModelType: ModelType.ACTION_PLANNER,
					body: {
						text: "Delegating the live lookup to a task agent.",
						toolCalls: [
							{
								id: "call-1",
								name: "SPAWN_AGENT",
				...
				},
				{
					...
					body: JSON.stringify({
						...
						messageToUser: "I started a task agent for that lookup.",
					}),
				},
			],
		});
		...
		await runV5MessageRuntimeStage1({
			runtime,
			message: makeMessage("what is the current BTC price in USD?"),
			state: makeState(),
			responseId: RESPONSE_ID,
		});

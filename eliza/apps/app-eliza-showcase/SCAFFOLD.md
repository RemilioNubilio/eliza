...
- `eliza-showcase` — the npm-style package name. Use lowercase, hyphenated. Scope it under `@elizaos/` for first-party apps, otherwise pick a sensible scope.
- `Eliza Showcase` — the human-readable name shown in the dashboard.
...
```
APP_CREATE_DONE {"appName":"<eliza-showcase>","files":["<list>"],"tests":{"passed":<n>,"failed":0},"lint":"ok","typecheck":"ok","description":"<one factual sentence>"}
```
